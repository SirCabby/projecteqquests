...
-- Thex Dagger turn-in (Classic_OldWorldDrops legacy toggle): Sealed Letter -> Thex Dagger.
-- Tolon is spawned by Inkeeper Freegraze (say "tolon nurbyte") and depops after 150s.
function event_trade(e)
	local item_lib = require("items");
	if(eq.is_content_flag_enabled("Classic_OldWorldDrops") and item_lib.check_turn_in(e.trade, {item1 = 18841})) then
		e.self:Say("So the Princess yet lives, and sends you to me. You have done the Silent Watch a great service. Take this -- the Thex Dagger, borne in the King's name. Now show yourself the door, and speak of it to no one.");
		e.other:SummonItem(13353); -- Item: Thex Dagger
		e.other:Ding();
